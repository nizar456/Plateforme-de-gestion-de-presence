...
function ProfessorCoursesPage() {
  ...
  const fetchCourses = async () => {
    setLoading(true)
    try {
      // In a real app, we would filter by the logged-in professor's ID
      // For now, we'll just fetch all courses
  ...
  // Sample data for demonstration (will be replaced by API data in production)
  const sampleCourses = [
    {
      id: "CS101",
    ...
    <ProfessorLayout>
      <div className="py-6">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h1 className="text-2xl font-semibold text-gray-900 dark:text-white">Mes Cours</h1>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Gérez vos cours, matériels pédagogiques et étudiants.
          </p>
        </div>
                    ...
                    <div className="px-6 py-5 flex items-center justify-between">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          <p className="text-lg font-medium text-blue-600 dark:text-blue-400 truncate">{course.name}</p>
                  ...
                  <div>
                    <p className="text-sm text-gray-700 dark:text-gray-300">
                      ...
                      <span className="font-medium">
                        {Math.min((currentPage + 1) * pageSize, displayCourses.length)}
                      </span>{" "}
                      sur <span className="font-medium">{displayCourses.length}</span> cours
                    </p>
                  </div>
                  <div>
